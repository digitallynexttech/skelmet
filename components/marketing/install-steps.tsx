        ...
        <div>
          ...
          <ol className="grid gap-3.5 sm:grid-cols-2">
            ...
            {INSTALL_STEPS.map((step) => (
              <li
                key={step.n}
                className="rounded-tile bg-void border border-white/[0.09] p-5 sm:p-6"
              >
                <div className="font-display text-blaze mb-3 text-[32px] leading-none sm:text-[34px]">
                  {step.n}
                </div>
                <h3 className="text-bone mb-1.5 text-[15.5px] font-bold sm:text-base">
                  {step.title}
                </h3>
                <p className="text-ash text-[13px] leading-[1.52] sm:text-[13.5px]">{step.body}</p>
              </li>
            ))}
          </ol>
        </div>
