...
const TIERS = [
  ...
  },
  {
    ...
    hex: "#D4FF3D",
    range: "15+ referrals",
    perks: ["₹500 per order that ships", "A custom-colour mount, on us", "Your name on the rider wall"],
    current: false,
  },
]
    ...
    <>
      <div className="grain relative overflow-hidden border-b border-white/[0.07] px-5 pt-14 pb-14 sm:px-8 xl:px-14">
        <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_480px]">
          <div>
            ...
            <h1 className="mb-6 font-display text-[52px] leading-[1.0] text-bone uppercase sm:text-[74px] xl:text-[96px]">
              Get paid for
              <br />
              having <span className="text-acid">taste</span>
            </h1>
            <p className="mb-8 max-w-[500px] text-[16px] leading-[1.62] text-ash text-pretty sm:text-[17.5px]">
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              <span className="font-mono text-[11.5px] tracking-[0.14em] text-dim uppercase">
                No cap · no expiry
              </span>
            </div>
          </div>
          ...
          <div className="relative aspect-square w-full">
            <div className="absolute inset-[8%] animate-spin-rev rounded-full border border-dashed border-acid/28" />
            ...
            />
            <span className="absolute top-[14%] right-[4%] rounded-lg border border-acid/30 bg-carbon/85 px-3.5 py-2.5 font-mono text-[10px] tracking-[0.12em] text-acid">
              + &#8377;{REFERRAL_REWARD}
            </span>
            <span className="absolute bottom-[22%] left-[2%] rounded-lg border border-acid/30 bg-carbon/85 px-3.5 py-2.5 font-mono text-[10px] tracking-[0.12em] text-acid">
              + &#8377;{REFERRAL_REWARD}
            </span>
          </div>
        </div>
        ...
        <ol className="grid gap-5 lg:grid-cols-3">
          {STEPS.map((step, i) => (
            <li
              key={step.n}
              className={cn(
                "rounded-card border bg-void p-7 sm:p-8",
                i === 2
            ...
            >
              <div
                className={cn(
                  "mb-5 font-display text-[52px] leading-none sm:text-[60px]",
                  i === 2 ? "text-acid" : "text-acid/30",
                )}
              >
                {step.n}
              </div>
              <h3 className="mb-3 font-display leading-[1.08] text-[26px] text-bone uppercase sm:text-[28px]">
                {step.title}
              </h3>
              <p className="text-[15px] leading-[1.6] text-ash">{step.body}</p>
            </li>
          ))}
        </ol>
      </Section>
      ...
      <Section id="code">
        <div className="grid gap-6 lg:grid-cols-[480px_minmax(0,1fr)]">
          ...
          <div className="rounded-card border border-white/10 bg-carbon p-6 sm:p-8">
            <div className="mb-6 flex items-center justify-between gap-4">
              <h2 className="font-display leading-[1.08] text-[26px] text-bone uppercase sm:text-[28px]">
                Your invites
              </h2>
            ...
            <div className="-mx-2 overflow-x-auto px-2">
              <div className="min-w-[460px]">
                <div className="grid grid-cols-[1.4fr_1fr_1fr_0.9fr] gap-4 border-b border-white/10 pb-3.5 font-mono text-[10.5px] tracking-[0.16em] text-dim uppercase">
                  <span>Who</span>
                  <span>Sent</span>
                  <span>Status</span>
                ...
                {INVITES.map((inv) => (
                  ...
                  >
                    <span className="truncate text-bone">{inv.who}</span>
                    <span className="font-mono text-[12.5px] text-ash">{inv.sent}</span>
                    <span>
                    ...
                    <span
                      className={cn(
                        ...
                        inv.earned === "-" ? "text-faint" : inv.tone === "acid" ? "text-acid" : "text-ash",
                      )}
                    >
                      {inv.earned}
                    </span>
      ...
      <Section className="border-t border-white/[0.07] bg-carbon">
        ...
        <div className="grid gap-5 lg:grid-cols-3">
          {TIERS.map((tier) => (
            <article
              ...
              className={cn(
                "rounded-card border bg-void p-7 sm:p-8",
                tier.current ? "border-blaze/35 bg-[linear-gradient(160deg,rgb(255_90_31_/_0.1),transparent_56%)]" : "border-white/[0.09]",
              )}
            >
              <div className="mb-3 flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <span className="size-3.5 rounded-full" style={{ backgroundColor: tier.hex }} />
                  <h3 className="font-display leading-[1.08] text-[24px] text-bone uppercase sm:text-[26px]">
                    {tier.name}
                  </h3>
                </div>
                {tier.current ? <Badge variant="blaze">You</Badge> : null}
              </div>
              <div className="mb-6 font-mono text-[11.5px] tracking-[0.14em] text-dim uppercase">
                {tier.range}
              </div>
              <ul className="flex flex-col gap-3">
                {tier.perks.map((perk, i) => (
                  <li
                    key={perk}
                    className={cn("text-[14.5px]", i === 0 && tier.current ? "text-bone" : "text-ash")}
                  >
                    {perk}
                  </li>
                ))}
        ...
        <div className="mt-8 flex items-start gap-3 rounded-tile border border-white/[0.08] bg-void p-5 sm:p-6">
          <Info className="mt-0.5 size-[18px] shrink-0 text-dim" strokeWidth={1.8} />
